        ...
        {jobs.length === 0 ? (
          ...
          <div className="space-y-6">
            {sortedJobs.slice(0, 4).map((job) => {
              ...
              const isRecent =
                Date.now() - new Date(job.createdAt).getTime() < 60 * 60 * 1000;

              return (
                <div
                  key={job._id}
                  className={`relative flex flex-col md:flex-row md:items-start gap-4 p-4 sm:p-6 rounded-xl shadow-md hover:shadow-lg transition cursor-pointer ${
                    darkMode ? "bg-gray-800 text-white" : "bg-white text-black"
                  }`}
                >
                  <span
                    className={`absolute top-4 left-4 text-xs px-2 py-1 rounded-md font-medium ${
                      isRecent
                        ? "bg-green-100 text-green-600"
                        : "bg-blue-100 text-[#0096ff]"
                    }`}
                  >
                  ...
                  <div
                    ...
                    className="flex flex-row items-start gap-4 w-full mt-8 sm:mt-6"
                  >
                    <img
                      src={job.thumbnail}
                      alt="company"
                  ...
                  <div className="flex justify-between md:justify-center md:flex-col gap-1 md:items-end text-sm mt-2 md:mt-0 whitespace-nowrap">
