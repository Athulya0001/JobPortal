    ...
    return (
        <section className={`py-10 px-4 transition duration-300 ${darkMode ? 'bg-gray-900 text-white' : 'bg-white text-black'}`}>
            <div className="max-w-6xl mx-auto">
                <h2 className="text-3xl font-bold mb-6 text-center text-[#0096FF]">Recommended Jobs for You</h2>

                {recommendedJobs.length === 0 ? (
                    <p className="text-center text-gray-400">No recommended jobs based on your skills yet.</p>
                ) : (
                    <div className="space-y-4">
                        {recommendedJobs.slice(0, 4).map((job) => {
                            ...
                            return (
                                <div
                                    key={job._id}
                                    className={`relative flex flex-col gap-4 p-5 rounded-xl shadow-md hover:shadow-lg transition cursor-pointer border border-transparent hover:border-[#007ACC] ${darkMode ? 'bg-gray-800 text-white' : 'bg-white text-black'}`}
