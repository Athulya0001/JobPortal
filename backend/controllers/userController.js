...
export const register = async (req, res) => {
  console.log("Hex Dump:", Buffer.from(JSON.stringify(req.body)).toString("hex"));
  ...
  try {
    ...
    // Sanitize input
    const sanitizeInput = (input) => input.replace(/[^\x00-\x7F]/g, "");
    const sanitizedName = sanitizeInput(name);
    const sanitizedEmail = sanitizeInput(email);
    const sanitizedProfileImage = sanitizeInput(profileImage);
    const sanitizedRole = sanitizeInput(role).substring(0, 50); // Limit length

    const newUser = new User({ clerkId, name: sanitizedName, email: sanitizedEmail, profileImage: sanitizedProfileImage, role: sanitizedRole });
    await newUser.save();
